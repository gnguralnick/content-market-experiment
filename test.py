...
def test(topics: np.ndarray, varied_param: str, num_agents: int | list[int], num_influencers: int | list[int],
         topic_interest_func: Callable[[np.ndarray], float] | list[Callable[[np.ndarray], float]],
    ...
    param_names = ['num_agents', 'num_influencers', 'topic_interest_func',
    ...
    params = [num_agents, num_influencers, topic_interest_func,
                consumer_attention_bound, consumer_external_interest_prob, consumer_delay_sensitivity,
                influencer_attention_bound, influencer_delay_sensitivity, production_rate, external_production_rate]
    ...
    for combo in param_combinations:
        ...
        AgentType = ConsumerProducer
        if use_imperfect_information:
            AgentType = ImperfectConsumerProducer
        for i in range(agents):
            market.add_agent(AgentType(topic_interest_func, cons_att_bound, cons_ext_prob, cons_delay_sens, init_following_rates_method))
